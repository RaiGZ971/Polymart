import { ChevronLeft, ChevronRight } from "lucide-react";
import { useState } from "react";

const defaultImages = [
  "https://picsum.photos/id/1011/400/300",
  "https://picsum.photos/id/1012/400/300",
  "https://picsum.photos/id/1013/400/300",
  "https://picsum.photos/id/1015/400/300",
  "https://picsum.photos/id/1016/400/300",
];

export default function ImageCarousel({ images = [], productName = "Product" }) {
  const [selected, setSelected] = useState(0);
  
  // Use provided images or fall back to default images
  const displayImages = images && images.length > 0 
    ? images.map(img => typeof img === 'string' ? img : img.image_url || img.url)
    : defaultImages;

  const handlePrev = () =>
    setSelected((selected - 1 + displayImages.length) % displayImages.length);
  const handleNext = () => setSelected((selected + 1) % displayImages.length);

  return (
    <div className="flex flex-col items-center gap-2 w-full">
      <div className="w-full aspect-square mb-2">
        <img
          src={displayImages[selected]}
          alt={`${productName} ${selected + 1}`}
          className="w-full h-full object-cover rounded-xl"
        />
      </div>
      <div className="flex flex-row items-center gap-2">
        <button
          className="bg-white rounded-full p-2 shadow hover:bg-gray-100"
          onClick={handlePrev}
        >
          <ChevronLeft size={24} />
        </button>
        <div className="flex flex-row gap-2">
          {displayImages.map((img, idx) => (
            <img
              key={img}
              src={img}
              alt={`${productName} thumbnail ${idx + 1}`}
              className={`w-16 h-16 object-cover rounded-lg cursor-pointer border-2 ${
                selected === idx ? "border-primary-red" : "border-transparent"
              }`}
              onClick={() => setSelected(idx)}
            />
          ))}
        </div>
        <button
          className="bg-white rounded-full p-2 shadow hover:bg-gray-100"
          onClick={handleNext}
        >
          <ChevronRight size={24} />
        </button>
      </div>
    </div>
  );
}
